...
reg [7:0] good_cnt_limit;

wire good_cnt_limit_reached;
...
always @(*) begin
  ...
  STATE_CGS: begin
               state_good <= &(cgs_ready | cfg_lanes_disable);
               good_cnt_limit = 'hff;
             end
...
assign good_cnt_limit_reached = good_counter == good_cnt_limit;

always @(posedge clk) begin
  if (reset) begin
    good_counter <= 'h00;
  end else if (state_good == 1'b1) begin
    if (good_cnt_limit_reached) begin
      good_counter <= 'h00;
    end else begin
